...
async def help(ctx: lightbulb.Context):
    cogs = [ctx.bot.get_plugin(cog) for cog in ctx.bot.plugins if
            ctx.bot.get_plugin(cog).name != "Help Commands" and ctx.bot.get_plugin(cog).all_commands]

    embed = hikari.Embed(
        description="For the best experience, Cordle should be used with slash commands to prevent spoilers. "
                    "Commands run with bot mention will not display author-only messages.")
